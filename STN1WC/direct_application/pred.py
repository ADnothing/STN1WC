...
load_epoch = 0
init_data_gen()
net_path = "net_save_sdc2_full_train.dat"

# === INFERENCE EXECUTION BLOCK ===
if(1):
	#Initialize CIANNA model
	cnn.init(in_dim=i_ar([sky_size,sky_size,freq_size]), #Input dimensions (3D)
				in_nb_ch=1, #Number of input channels
				out_dim=1+max_nb_obj_per_image*(7+nb_param), #YOLO output dimension
				bias=0.1,
				b_size=16, #Batch size
				comp_meth='C_CUDA',
				dynamic_load=1,
				mixed_precision="FP16C_FP32A",
				inference_only=1,
				adv_size=30
		)
