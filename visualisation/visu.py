...
def sources_visu(hdul, cat_path):
	...
	for idx in range(len(sou_id)):
		...
		cutout_2d = cube_data[
					max(0, z_pix - 15):min(cube_data.shape[0], z_pix + 15),
		...
		title_str = f"LYC_{source_id:04d}  O = {int(round(objness[idx]*100))}%  P = {int(round(proba[idx]*100))}%"
		...
		freq_start = max(0, z_pix - 10)
		freq_end = min(cube_data.shape[0], z_pix + 10)
		...
		plt.tight_layout()
		plt.savefig(f"./images/source_{source_id:04d}.pdf", dpi=200)
		plt.close(fig)
